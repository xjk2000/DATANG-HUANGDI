...
```bash
# 开始执行时立即调用（心跳 + 看板双写）
python3 __REPO_DIR__/scripts/agent_heartbeat.py pulse jiangzuo --status working --task "<任务简述>" --edict CL-xxx --phase "开始执行"
python3 __REPO_DIR__/scripts/kanban_update.py progress CL-xxx "将作监正在执行" "架构设计🔄|核心编码|单元测试|集成验证"

# 执行过程中定期更新心跳（避免被判定为卡死）
python3 __REPO_DIR__/scripts/agent_heartbeat.py pulse jiangzuo --status working --task "<当前步骤>" --edict CL-xxx --phase "<阶段>"
