...
**立即通过 agent_invoke.py 唤起给事中**，让其对敕令进行全面漏洞排查：

```bash
python3 __REPO_DIR__/scripts/agent_invoke.py invoke shizhong jishizhong "请对敕令 CL-xxx 进行全面漏洞排查" --edict CL-xxx
```
...
**准奏后必须立即通过 agent_invoke.py 唤起尚书令**，将准奏的敕令转交执行：

```bash
python3 __REPO_DIR__/scripts/agent_invoke.py invoke shizhong shangshuling "准奏敕令 CL-xxx，请接令派发执行" --edict CL-xxx
```
