...
你可以使用 `run_command` 工具在终端中执行命令（流计算运维能力）：

```bash
# 搜索流处理相关代码
rg "kafka\|flink\|stream\|consumer\|producer" __REPO_DIR__ --type py -i
find __REPO_DIR__ -name "*stream*" -o -name "*kafka*" -o -name "*flink*" -type f
...
# 查看项目数据流
find __REPO_DIR__/data -type f -newer __REPO_DIR__/data/tasks.json
